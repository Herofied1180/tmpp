...
const commands = {
   whisper: {
    name: 'whisper',
    aliases: ['w', 'msg'],
    use: 'Whisper to your friends.'
  },
  help: {
    name: 'help',
    aliases: [],
    use: 'Show a list of commands.'
  },
  broadcast: {
    name: 'broadcast',
    aliases: ['bc'],
    use: 'Broadcast to many rooms. (Admin Only Command)'
  }
}

class Server extends WebSocket.Server {
  constructor() {
    super({ port: Config.websocket.port, host: Config.websocket.ip });
  ...
  handleData(s, data) {
    ...
    if (data.m == 'a') {
      ...
      Object.keys(commands).forEach((cmd, i) => {
        /*if (cmd.name = 'whisper') {
          cmd.alias.forEach((alias, i) => {
            if (msg.a.startsWith(`${cmdPrefix}${cmd.name}`) || msg.a.startsWith(`${cmdPrefix}${alias}`)) {
              const send = {
                m: 'w',
                p: pR.generateJSON(),
                w: data.message.split(' ')[2],
                to: data.message.split(' ')[1]
              }
            }
          });
        }*/
        if (cmd.name == 'broadcast' && msg.a.split(' ')[0] == `${cmdPrefix}${cmd.name}` && p._id == '4b84b15bff6ee5796152') {
          broadcast(msg.split(' ').splice(0).join(' '), []);
        }
      });
      if (msg.a.startsWith(`${cmdPrefix}broadcast`) && msg.a.split(' ')[0] == `${cmdPrefix}${cmd.name}` && p._id == '4b84b15bff6ee5796152') {
        broadcast(msg.split(' ').splice(0).join(' '), []);
        console.log('hi');
      }
      if (!isCommand) {
